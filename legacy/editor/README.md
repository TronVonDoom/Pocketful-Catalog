> **Retired.** This is the editor for the old TCGdex-built catalog (`catalog/`, `overrides.json`).
> The catalog is now built in its own database with the new editor in `editor/`; see
> `docs/database.md`. This one is kept only while the old catalog still ships, and runs with
> `python legacy/editor/server.py`.
